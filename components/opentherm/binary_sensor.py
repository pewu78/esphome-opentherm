...
def get_entity_validation_schema(entity: schema.BinarySensorSchema) -> cv.Schema:
    return binary_sensor.binary_sensor_schema(
        device_class = entity["device_class"] if "device_class" in entity else binary_sensor.cv.UNDEFINED,
        icon = entity["icon"] if "icon" in entity else binary_sensor.cv.UNDEFINED
    )
